...
describe("simple", function() {
  ...
  it("should get collection of resources", function() {
    ...
    $httpBackend.expect("GET", "/books").respond({
      _embedded: {
        books: mockBooks
      }
    });
    ...
    halClient.$get("/").then(function(resource) {
      resource.$request().$getCollection("books").then(books => {
  ...
  it("should get with options", function() {
    ...
    $httpBackend
      .expect(
        ...
        "/books",
        null,
        headers => headers["some-header"] === "aaa"
      )
      .respond({});
    ...
    halClient.$get("/").then(function(resource) {
      resource
        .$request()
        .$get("books", null, { headers: { "some-header": "aaa" } })
        .then(books => {
  ...
  it("should get link by templated url", function() {
    ...
    $httpBackend.expect("GET", "http://example.com/item/1").respond({
      id: 1,
      _links: {
        self: "/item/1"
      }
  ...
  it("should get build href from a templated link", function() {
    $httpBackend.expect("GET", "https://example.com/").respond({
      root: true,
      _links: {
        self: "/",
        item: {
